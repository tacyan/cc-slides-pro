...
- 金額・割合・日数などの巨大数字は、途中改行・欠け・単位の分離を絶対に許可しない。`¥198,000`、`4,500万円`、`0.1%` は1つの塊として読めることを生成後に必ず確認する
...
- 金額や数字が主役のスライドでは、数字・カンマ・単位・税込/税抜ラベルが重なっていないか、途中で改行されていないかを最優先で確認する。崩れていたら完成扱いにせず、フォントサイズ・カラム幅・レイアウトを直して再生成する
